...
function init (env, ctx) {
  ...
  try {
    ...
    tokenStore = require('nightscout-connect/lib/sources/minimedcarelink/token-store').create();
  } catch (e) {
  ...
  router.post('/exchange', ctx.authorization.isPermitted('admin:api:carelink:auth'), function (req, res) {
    ...
    auth.exchangeCodeForTokens(session.ssoConfig, session.baseUrl, code, session.codeVerifier, axios)
      .then(function (loginData) {
        ...
        var savePromise = tokenStore
          ? tokenStore.save(loginData).then(function () {
            console.log('[CareLink API] Tokens saved to MongoDB');
          })
          : Promise.resolve();
        ...
        return savePromise.then(function () {
          ...
          console.log('[CareLink API] Login successful, tokens saved to MongoDB');

          res.json({
            success: true,
            message: 'CareLink authentication successful. Tokens saved to MongoDB.',
  ...
  router.get('/status', ctx.authorization.isPermitted('admin:api:carelink:read'), function (req, res) {
    ...
    tokenStore.load()
      .then(function (data) {
        if (!data) {
          return res.json({ configured: false, message: 'No tokens in MongoDB. Use the admin UI to authenticate.' });
        }

        var expired = oauth.isTokenExpired(data.access_token);
        var region = oauth.detectRegion(data);
        var payload = oauth.decodeJwtPayload(data.access_token);
        var expiresIn = payload && payload.exp ? Math.floor((payload.exp * 1000 - Date.now()) / 60000) : null;
      ...
      .catch(function (err) {
        res.json({ configured: false, message: 'Failed to load tokens from MongoDB: ' + err.message });
      });
  });
